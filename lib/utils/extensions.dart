...
String convertDateToStringDayName(DateTime time) =>
    DateFormat("EEEE", 'in_ID').format(time ?? DateTime.now());

String getTimeOfDate(String time) =>
    DateFormat.Hm().format(DateTime.parse(time) ?? DateTime.now());
...
Future<String> downloadImage({String image}) async {
  if (await _requestPermission(Permission.storage)) {
    var url = image ?? 'https://venus.bisnisomall.com/images/blank.png';
...
void shareProductImageText(
  ...
  Share.shareFiles(['$documentDirectory/product.png'],
      text:
      userData.reseller != null ?
          "Produk $nameProduk - https://store.ekomad.id/wpp/productdetail/${userData.reseller.slug}/$productSlug/$idProduct" :
          "Produk $nameProduk - https://store.ekomad.id/productdetail/$idProduct");
}
...
double commisionConvert(int productPrice, int commision) {
  // (HARGA KOMISI/HARGA  * 100)
  debugPrint(productPrice.toString());
  debugPrint(commision.toString());
  return ((commision / productPrice) * 100);
}
