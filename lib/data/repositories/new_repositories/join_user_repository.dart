...
class JoinUserRepository {
  ...
  Future<void> updateProfileSupplier({
    ...
    if (await AppExt.hasConnection()) {
      ...
      //Buat Web
      Uint8List data = kIsWeb ? await logoBytes.readAsBytes() : null;
      List<int> logoWeb = kIsWeb ? data.cast() : null;
      var formData = new FormData.fromMap({
        "name": name,
        "phonenumber": phoneNumber,
        "logo": logo != null || logoBytes != null
            ? kIsWeb
                ? MultipartFile.fromBytes(logoWeb, filename: "logo")
  ...
  Future<GeneralResponse> addProductSupplier(
      ...
      @required List<dynamic> productPhotoByte,
    ...
    if (await AppExt.hasConnection()) {
      ...
      if (!kIsWeb) {
        ...
        for (var file in productPhotoByte) {
          ...
          var multipartFile =
              MultipartFile.fromBytes(byte, filename: "product_photo");
          uploadPhotoByte.add(multipartFile);
        }
      }

      FormData formDataGeneral = FormData.fromMap({
        "name": name,
  ...
  Future<FeeServiceAddProductResponse> getFeeServiceAddProduct(
      {@required int price}) async {
    final token = await _authenticationRepository.getToken();
    final response = await _provider.get(
        "/supplier/product-submissions/costs/check?price=$price",
        headers: {
          HttpHeaders.authorizationHeader: 'Bearer $token',
          HttpHeaders.contentTypeHeader: 'text/plain',
          'ADS-Key': _adsKey
    ...
    return FeeServiceAddProductResponse.fromJson(response);
  }
