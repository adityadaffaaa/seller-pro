    ...
    if (stateCart.isEmpty) {
      emit(state.copyWith(
          cart: List<CartResponseElement>.from(state.cart)
            ..add(CartResponseElement(supplier: product.supplier, reseller: product.resellerWebStore, products: [
              ProductsCart(
      ...
      for (var i = 0; i < stateCart.length; i++) {
        ...
        for (var p = 0; p < currentProducts.length; p++) {
          ...
          // kalo supplier sama
          if (currentProducts[p].productId == productId && currentReseller.id == product.resellerWebStore.id){
          // if (theSame){
            // inc qty
            ...
            emit(state.copyWith(
                cart: List<CartResponseElement>.from(state.cart)..removeAt(i)
                  ..add(CartResponseElement(supplier: product.supplier,reseller: product.resellerWebStore, products: result))));
            break;
          } else if (currentProducts[p].productId != productId && currentReseller.id == product.resellerWebStore.id) {
            ...
            emit(state.copyWith(
                cart: List<CartResponseElement>.from(state.cart)..removeAt(i)..add(CartResponseElement(supplier: product.supplier,reseller: product.resellerWebStore, products:
            ...
            break;
          } else if (currentReseller.id != product.resellerWebStore.id){
            final sameResellerId = List<CartResponseElement>.from(state.cart).map((e) => e.reseller.id == product.resellerWebStore.id).toList().contains(true);
