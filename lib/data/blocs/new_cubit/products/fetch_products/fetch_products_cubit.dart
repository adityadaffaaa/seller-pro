...
class FetchProductsCubit extends Cubit<FetchProductsState> {
  ...
  Future<void> fetchReadyToCook() async {
    emit(FetchProductsLoading());
    try {
      final response = await _productsRepository.fetchReadyToCook();
      emit(FetchProductsSuccess(response.data));
    } catch (error) {
      emit(FetchProductsFailure(error.toString()));
    }
  ...
  Future<void> fetchRestoPanen() async {
    emit(FetchProductsLoading());
    try {
      final response = await _productsRepository.fetchRestoPanen();
      emit(FetchProductsSuccess(response.data));
    } catch (error) {
      emit(FetchProductsFailure(error.toString()));
    }
  ...
  Future<void> fetchCatering() async {
    emit(FetchProductsLoading());
    try {
      final response = await _productsRepository.fetchCatering();
      emit(FetchProductsSuccess(response.data));
    } catch (error) {
      emit(FetchProductsFailure(error.toString()));
    }
