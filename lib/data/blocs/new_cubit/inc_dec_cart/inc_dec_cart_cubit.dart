...
class IncDecCartCubit extends Cubit<IncDecCartState> {
  ...
  void initialization(List<CartResponseElement> cart, bool isSupplier) {
    ...
    for (var c = 0; c < cart.length; c++) {
      ...
      for (var p = 0; p < cart[c].products.length; p++) {
        ...
        final _productVariant = cart[c].products[p].product.productVariant.isEmpty ? null : cart[c].products[p].product.productVariant[0];
  ...
  void increment({int value, int indexStore, int indexItem, List<CartStore> myStore}) {
    ...
    if (value != null) {
      ...
      var tempItem = store[indexStore].item[indexItem].copyWith(
          qty: incrementQty,
          subTotal: 
          store[indexStore].item[indexItem].variant != null ?
          store[indexStore].item[indexItem].variant.variantSellPrice :
          store[indexStore].item[indexItem].subTotal);
  ...
  void decrement( {int value, int indexStore, int indexItem, List<CartStore> myStore}) {
    final _state = this.state;
    var store = myStore ?? _state.store;
    var item = store[indexStore].item;
  ...
  void updateCheckedStoreItem({
    ...
    if (store[indexStore].item[indexItem].checked == value &&
      ...
      final tempItem1 = store[indexStore]
          .item[indexItem]
          .copyWith(qty: store[indexStore].item[indexItem].qty, subTotal: 
          (
            store[indexStore].item[indexItem].variant != null ?
          store[indexStore].item[indexItem].variant.variantSellPrice :
            store[indexStore].item[indexItem].price) * store[indexStore].item[indexItem].qty);
      ...
      if (store[indexStore].item[indexItem].qty > 1) {
        ...
        final tempTotal = store[indexStore].total -
            (store[indexStore].item[indexItem].qty) *
                ...
                (store[indexStore].item[indexItem].variant != null ?
          store[indexStore].item[indexItem].variant.variantSellPrice : store[indexStore].item[indexItem].price);
