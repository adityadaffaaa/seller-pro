...
import 'package:marketplace/ui/screens/mobile/nav/transaksi/widgets/bs_transaksi_status.dart';
import 'package:marketplace/ui/screens/mobile/nav/transaksi/widgets/bs_transaksi_tanggal.dart';
            ...
            Builder(
              builder: (context) {
                final isFilterStatusFilled = context.select(
                        (TransaksiFilterStatusCubit cubit) =>
                            cubit.state.kategori) !=
                    "Semua Status";
                ...
                final isFilterKategoriFilled = !context
                    .select((TransaksiFilterKategoriCubit cubit) => cubit.state)
                    .kategori
                    .contains("Semua Kategori");
              ...
              builder: (context, state) {
                return FilterElement(
                  onTap: () {
                    ...
                    BsKategori().showBsStatus(context, onPressed: () {
                      final status = context
                          ...
                          .index;
                      final kategori =
                          context.read<TransaksiFilterKategoriCubit>().state;
                      final tanggal =
                          context.read<TransaksiFilterTanggalCubit>().state;
                      context.read<FetchTransactionNewBloc>().add(
                          TransactionFetched(
                              status: status,
                  ...
                  child: Text(
                    state.kategori,
                    style: AppTypo.caption.copyWith(
                        color: state.kategori.contains("Semua Kategori")
                            ? Colors.grey
                            : Theme.of(context).primaryColor),
                  ),
                );
